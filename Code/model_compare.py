...
def main():
    ...
    # get X, Y data
    (X1, Y1) = read_file(file_path)
    (X2, Y2) = read_file(file_path)
    ...
    for i in range(epoch):
        ...
        #swap X_test, Y_test with random samples from X2, Y2
        old_metal_list = [k for k, l in enumerate(Y_test) if l == 1]
        old_none_metal_list = [k for k, l in enumerate(Y_test) if l!= 1]

        X_test = [X_test[k] for k in old_none_metal_list]
        Y_test = [Y_test[k] for k in old_none_metal_list]

        n = len(old_metal_list)
        new_metal_list = np.random.randint(0,100, n)
        new_metal_X = [X2[k] for k in new_metal_list]
        new_metal_Y = [Y2[k] for k in new_metal_list]


        X_test.extend(new_metal_X)
        Y_test.extend(new_metal_Y)
    ...
    plot_confusion_matrix(rf_conf_matrix, classes)
    #plot_confusion_matrix(gb_conf_matrix, range(n_label))
    #plot_confusion_matrix(gb_conf_matrix, classes)
    #plot_confusion_matrix(knn_conf_matrix, range(n_label))
